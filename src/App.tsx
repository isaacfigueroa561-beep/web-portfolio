...
function WebProjectPanel({
  ...
  return (
    <motion.div
      ...
      className="relative w-full flex flex-col md:flex-row border-b border-[#1a1a1a]"
    ...
    >
      ...
      <div
        className="relative flex flex-col justify-between md:w-[38%] flex-shrink-0 px-8 md:px-14 pt-14 pb-12 z-10"
        style={{
          background: "#0D0D0D",
          borderRight: "1px solid #1a1a1a",
        }}
      >
        {/* Orange accent bar — animates in on hover */}
        <motion.div
        ...
        >
          {project.urlDisplay ?? project.client}
        </div>
      </div>
      ...
      >
        {heroImage ? (
          ...
          /* Color panel for projects without a hero image */
          <div
            className="absolute inset-0 flex items-center justify-center overflow-hidden"
            style={{ background: project.bg }}
          >
            {/* Ghost project name — editorial typographic treatment */}
            <motion.div
              animate={{ scale: hovered ? 1.03 : 1, opacity: hovered ? 0.12 : 0.09 }}
              transition={{ duration: 1.1, ease: [0.25, 0.46, 0.45, 0.94] }}
              style={{
                fontFamily: "var(--font-serif)",
                fontWeight: 800,
            ...
            {/* Grid lines — suggest UI / precision */}
            <div
              className="absolute inset-0 pointer-events-none"
              style={{
                backgroundImage:
...
function CarouselModal({
  ...
  const handleDeviceTilt = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    const rect = stageRef.current?.getBoundingClientRect();
    if (!rect || !devicesRef.current) return;
    const x = (e.clientX - rect.left) / rect.width - 0.5;
    const y = (e.clientY - rect.top) / rect.height - 0.5;
    devicesRef.current.style.transform = `perspective(1300px) rotateX(${y * -5}deg) rotateY(${x * 7}deg)`;
  }, []);

  const handleDeviceLeave = useCallback(() => {
    if (devicesRef.current) {
      devicesRef.current.style.transform = "perspective(1300px) rotateX(0deg) rotateY(0deg)";
    }
  }, []);
        ...
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
                          ...
                          <div className="grid border-b border-[#1a1a1a]" style={{ gridTemplateColumns: `repeat(${project.caseStudy.stats.length}, 1fr)` }}>
                            {project.caseStudy.stats.map((stat, i) => (
                              <div key={i} className={`bg-[#111] px-5 py-4 flex flex-col gap-1 ${i < project.caseStudy!.stats!.length - 1 ? "border-r border-[#1a1a1a]" : ""}`}>
                                ...
                                {stat.context && <span className="font-sans font-light text-[9px] text-[#F5F0E8]/25 leading-tight mt-0.5">{stat.context}</span>}
                              </div>
                            ))}
                          </div>
                        )}
            ...
            {project.deviceMockup ? (
              ...
              <div className="pb-16">
                ...
                >
                  ...
                  <div
                    ...
                    style={{ gap: 0, position: "relative", transition: "transform 0.12s ease-out", transformStyle: "preserve-3d" }}
                  >
                        ...
                        <div style={{
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              🔒 {project.urlDisplay ?? project.liveUrl}
                            </div>
                          </div>
                          <div style={{ width: 60 }} />
                        </div>
              ...
              /* ── Browser-only mockup — external website ── */
              <div className="pb-16">
                <div style={{
                  background: "#060606",
                  borderTop: "1px solid #111",
                  ...
                  {/* Radial glow — warm earthy tone */}
                  <div aria-hidden="true" style={{
                    position: "absolute", inset: 0, pointerEvents: "none",
                    background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(194,117,64,0.07) 0%, transparent 70%)",
                  }} />
                  ...
                  <div style={{ maxWidth: 960, margin: "0 auto", position: "relative" }}>
                    <div style={{
                      ...
                      background: "#0c0c0c",
                      overflow: "hidden",
                      boxShadow: "0 0 0 1px rgba(0,0,0,0.9), 0 48px 120px rgba(0,0,0,0.95), 0 16px 48px rgba(0,0,0,0.7)",
                    }}>
                      {/* macOS chrome */}
                      <div style={{
                        height: 42, background: "#161616",
                        borderBottom: "1px solid rgba(255,255,255,0.05)",
                        display: "flex", alignItems: "center",
                        padding: "0 18px", gap: 8, flexShrink: 0,
                      }}>
                        <div style={{ width: 13, height: 13, borderRadius: "50%", background: "#FF5F57", boxShadow: "0 0 0 0.5px rgba(0,0,0,0.3)" }} />
                        <div style={{ width: 13, height: 13, borderRadius: "50%", background: "#FEBC2E", boxShadow: "0 0 0 0.5px rgba(0,0,0,0.3)" }} />
                        <div style={{ width: 13, height: 13, borderRadius: "50%", background: "#28C840", boxShadow: "0 0 0 0.5px rgba(0,0,0,0.3)" }} />
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 px-6 md:px-14 pt-5">
                  <div className="flex flex-wrap gap-2">
                    {["Web Design", "Brand Identity", "Marketing"].map(tag => (
                      <span key={tag} style={{
                        fontFamily: "sans-serif", fontSize: 10,
                        letterSpacing: "0.15em", textTransform: "uppercase",
                        color: "rgba(245,240,232,0.3)",
...
function Home() {
  ...
  const projects: Project[] = [
    ...
    },
    {
      ...
      caseStudy: {
        stats: [
          { value: "3.92x", label: "Blended ROAS", context: "Reuzel, all paid channels 2024" },
          { value: "+25%", label: "Ecomm growth YoY", context: "Taza Chocolate, 2025" },
          { value: "11.2M", label: "Influencer reach", context: "Cappello's, single campaign" },
          { value: "34.7x", label: "SMS program ROI", context: "Taza, lifetime" },
        ],
    ...
    <div className="min-h-[100dvh] w-full bg-background text-foreground overflow-x-hidden selection:bg-[#FF4D00] selection:text-black font-sans rounded-none" style={{ cursor: "none" }}>
      ...
      <section id="work" className="w-full border-t border-[#1a1a1a]">
        ...
        >
          ...
          <div className="font-sans font-light text-sm text-muted-foreground tracking-widest">({String(projects.length).padStart(2, "0")})</div>
        </motion.div>

        {/* — WEBSITES — showcase grid */}
        {(() => {
          const webProjects = projects.filter(p => p.liveUrl);
          const designProjects = projects.filter(p => !p.liveUrl);
          return (
            <>
              <div className="border-t border-[#1a1a1a]">
                {/* Sub-label */}
                <div className="flex items-center gap-6 px-8 md:px-16 py-4">
                  <span className="font-sans font-light text-[9px] uppercase tracking-[0.35em] text-[#F5F0E8]/20">Websites</span>
                  <div className="flex-1 h-px bg-[#1a1a1a]" />
                  <span className="font-sans font-light text-[9px] text-[#F5F0E8]/20 tabular-nums">{webProjects.length}</span>
                </div>
                <WebProjectShowcase
              ...
              <div className="border-t border-[#1a1a1a] relative">
                {/* Sub-label */}
                <div className="flex items-center gap-6 px-8 md:px-16 py-4">
                  <span className="font-sans font-light text-[9px] uppercase tracking-[0.35em] text-[#F5F0E8]/20">Design &amp; Brand</span>
                  <div className="flex-1 h-px bg-[#1a1a1a]" />
                  <span className="font-sans font-light text-[9px] text-[#F5F0E8]/12 tabular-nums">{designProjects.length}</span>
                </div>
                ...
                <AnimatePresence>
                  {hoveredProject?.images?.[0] && (
                    <motion.div
                      ...
                      style={{ right: "4vw", top: "50vh", transform: "translateY(-50%)", cursor: "pointer" }}
                      onMouseEnter={() => { /* keep visible */ }}
                      onMouseLeave={() => setHoveredProject(null)}
                      onClick={() => {
                        const idx = projects.findIndex(p => p.name === hoveredProject!.name);
                ...
                {designProjects.map((project, i) => (
                  <motion.div
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setSelectedIndex(projects.indexOf(project)); } }}
                    onMouseEnter={() => setHoveredProject(project)}
                    onMouseLeave={() => setHoveredProject(null)}
                    data-testid={`card-project-${projects.indexOf(project)}`}
                  >
                    <div className="project-card-row flex items-center px-6 md:px-16 py-8 md:py-9 gap-4 md:gap-10 group-hover:bg-[#111] border-l-2 border-transparent group-hover:border-[#FF4D00] transition-all duration-200">
                      ...
                      <div className="hidden md:flex flex-col items-end gap-[5px] flex-shrink-0 min-w-[160px]" aria-hidden="true">
                        <span className="font-sans font-light text-[10px] uppercase tracking-[0.22em] text-[#F5F0E8]/40 text-right">{project.category}</span>
                        <span className="font-sans font-light text-[10px] text-[#F5F0E8]/25 text-right">{project.client}</span>
                      </div>
                      {!project.images?.length && (
                        <div className="hidden md:block w-6 h-6 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ backgroundColor: project.bg }} aria-hidden="true" />
                      )}
                      <span className="project-card-arrow font-sans text-base text-[#F5F0E8]/20 group-hover:text-[#FF4D00] group-hover:translate-x-2 transition-all duration-300 flex-shrink-0 select-none" aria-hidden="true">→</span>
                    </div>
                  </motion.div>
                ))}
              </div>
